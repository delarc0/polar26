...
import { BYGGMARK_PHOTOS } from "@/data/byggmarkPhotos";

/* -------------------------------------------------------------------------- */
/*  Edit these two lines to change the event name / intro shown on the page.   */
const EVENT_TITLE = "Byggmark Racing";
const EVENT_INTRO =
  "Photos from the day. Browse below and save the ones you like.";
/* -------------------------------------------------------------------------- */

const THUMB = (name: string) => `/images/byggmarkracing/thumbs/${name}.webp`;
const FULL = (name: string) => `/images/byggmarkracing/photos/${name}.jpg`;

export function ByggmarkGallery() {
  const [active, setActive] = useState<number | null>(null);

  const close = useCallback(() => setActive(null), []);
  const next = useCallback(
    () => setActive((i) => (i === null ? i : (i + 1) % BYGGMARK_PHOTOS.length)),
    [],
  );
  const prev = useCallback(
    () =>
      setActive((i) =>
        i === null ? i : (i - 1 + BYGGMARK_PHOTOS.length) % BYGGMARK_PHOTOS.length,
      ),
    [],
  );
